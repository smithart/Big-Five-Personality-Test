...
function scoreTest(event) {
	...
	eResult = 20 + Number(form.elements["q1"].value) - Number(form.elements["q6"].value) + Number(form.elements["q11"].value) - Number(form.elements["q16"].value) + Number(form.elements["q21"].value) - Number(form.elements["q26"].value) + Number(form.elements["q31"].value) - Number(form.elements["q36"].value) + Number(form.elements["q41"].value) - Number(form.elements["q46"].value);
	// Flexible/Agreeableness (10)
	aResult = 14 - Number(form.elements["q2"].value) + Number(form.elements["q7"].value) - Number(form.elements["q12"].value) + Number(form.elements["q17"].value) - Number(form.elements["q22"].value) + Number(form.elements["q27"].value) - Number(form.elements["q32"].value) + Number(form.elements["q37"].value) + Number(form.elements["q42"].value) + Number(form.elements["q47"].value);
	// Mature/non-Neuroticism (10) -- reverse +/-
	nResult = 2 + Number(form.elements["q4"].value) - Number(form.elements["q9"].value) + Number(form.elements["q14"].value) + Number(form.elements["q19"].value) - Number(form.elements["q24"].value) + Number(form.elements["q29"].value) + Number(form.elements["q34"].value) + Number(form.elements["q39"].value) + Number(form.elements["q44"].value) + Number(form.elements["q49"].value);

	// Add the test results to the score graph
	oBarCaption.textContent = "Adventurous: " + oResult;
	cBarCaption.textContent = "Reliable: " + cResult;
	eBarCaption.textContent = "Extroverted: " + eResult;
	aBarCaption.textContent = "Flexible: " + aResult;
	nBarCaption.textContent = "Mature: " + nResult;
}
